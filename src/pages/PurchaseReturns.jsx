...
export default function PurchaseReturns() {
  ...
  const { data: returns = [], isLoading } = useQuery({
    queryKey: ['invoices', 'debit_note'],
    queryFn: () => base44.entities.Invoice.filter({ invoice_type: 'debit_note' }, '-created_date'),
  });
  ...
  const handleOriginalInvoiceSelect = (invoiceId) => {
    ...
    if (invoice) {
      ...
      setFormData({
        ...formData,
        invoice_no: `DN/${year}/${String(counter).padStart(5, '0')}`,
  ...
  const columns = [
    ...
    },
    {
      header: 'Amount',
      render: (row) => (
        <p className="font-semibold text-orange-600">{formatCurrency(row.grand_total)}</p>
      ),
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-[#0F1724]">Purchase Returns / Debit Notes</h1>
          <p className="text-gray-500 mt-1">Return goods to suppliers and issue debit notes</p>
        </div>
        <div className="flex gap-3">
          <ExportButton
            ...
            columns={[
              { header: 'Debit Note No', accessor: 'invoice_no' },
      ...
      <Dialog open={showForm} onOpenChange={(open) => !open && resetForm()}>
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <Label>Select Original Purchase *</Label>
              <Select onValueChange={handleOriginalInvoiceSelect}>
                <SelectTrigger className="mt-1.5">
                  <SelectValue placeholder="Select purchase to return" />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Input
                ...
                placeholder="e.g., Defective goods, Quality issue"
                className="mt-1.5"
              />
            </div>

            <div className="p-4 bg-orange-50 rounded-xl border border-orange-100">
              <p className="text-sm text-gray-600">Debit Note Amount</p>
              <p className="text-2xl font-bold text-orange-600">{formatCurrency(formData.grand_total)}</p>
            </div>
